// Joint model: DD (softmax/SVR) + RDM-ambiguity (softmax/SVR)
// Gaussian copula on mu_raw (DD) and mu_raw (RDM)
// RDM includes ambiguity level (alott) and ambiguity aversion (beta) per trial
...
model {
  ...
  // --- RDM likelihood ---
  {
    ...
    for (t in 1:nt_rdm) {
      int sid = subid_rdm[t];
      real adjustedProb = fmin(1.0, fmax(0.001, pgain[t] - beta[sid] * (alott[t] / 2.0)));
