...
parameters {
  vector[4] pop_mu;                // am, mm_g, km, mm_d
  vector<lower=0>[4] tau;          // as, ms_g, ks, ms_d
...
transformed parameters {
  ...
  vector<lower=0>[ns] mu_d = exp(theta[, 4]);
}
...
model {
  ...
  for (t in 1:nt_g) {
    ...
    real log_svGamble = log(pgain[t]) + alph[s] * log(gain[t]);
