function result = fitmodel_rdm_softmax_ambiguity(indata)

% RESULT = fitmodel_rdm_svd(INDATA)
%
...
result           = struct;
result.data      = indata;
result.betalabel = {'mu','alpha', 'beta'}; 
result.inx       = [1     0.8   0];   %initial values for parameters
result.lb        = [0.01  0.1  -2]; %min values possible for design matrix
result.ub        = [20    1.8   2];   %max values
...
try
    [b, ~, exitflag, output, ~, ~, H] = fmincon(@setup_rdm_svd_model_ambiguity,result.inx,[],[],[],[],result.lb,result.ub,[],result.options,result);
    clear temp;
    [loglike, utildiff, logodds, probchoice] = setup_rdm_svd_model_ambiguity(b, result);
...
function [loglike, utildiff, logodds, probchoice] = setup_rdm_svd_model_ambiguity(x, data)

data.mu     = x(1);
data.alpha  = x(2);
data.beta   = x(3);

[loglike, utildiff, logodds, probchoice] = rdm_svd_model_ambiguity(data);

end


function [loglike, utildiff, logodds, probchoice] = rdm_svd_model_ambiguity(data)
...
p_adj = data.data(:,5) - data.beta .* (data.data(:,2)/2);   % adjusted subjective prob
p_adj = max(min(p_adj,1),0);                                % bound probability between 0 and 1
utilgamble = p_adj .* (data.data(:,4).^data.alpha);
