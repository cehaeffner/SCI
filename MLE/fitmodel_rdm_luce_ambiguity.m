function result = fitmodel_rdm_luce_ambiguity(indata)
...
try
    [b, ~, exitflag, output, ~, ~, H] = fmincon(@setup_rdm_luce_model_ambiguity,result.inx,[],[],[],[],result.lb,result.ub,[],result.options,result);
    clear temp;
    [loglike, utildiff, logodds, probchoice] = setup_rdm_luce_model_ambiguity(b, result);
...
function [loglike, utildiff, logodds, probchoice] = setup_rdm_luce_model_ambiguity(x, data)

data.mu     = x(1);
data.alpha  = x(2);
data.beta   = x(3); 

[loglike, utildiff, logodds, probchoice] = rdm_luce_model_ambiguity(data);

end


function [loglike, utildiff, logodds, probchoice] = rdm_luce_model_ambiguity(data)
