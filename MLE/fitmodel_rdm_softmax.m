function result = fitmodel_rdm_softmax(indata)
...
try
    [b, ~, exitflag, output, ~, ~, H] = fmincon(@setup_rdm_svd_model,result.inx,[],[],[],[],result.lb,result.ub,[],result.options,result);
    clear temp;
    [loglike, utildiff, logodds, probchoice] = setup_rdm_svd_model(b, result);
    ...
    % adding pseudo-R2
    p_null = mean(indata(:,6)); % Mean choice
...
function [loglike, utildiff, logodds, probchoice] = setup_rdm_svd_model(x, data)

data.mu     = x(1);
data.alpha  = x(2);

[loglike, utildiff, logodds, probchoice] = rdm_svd_model(data);

end


function [loglike, utildiff, logodds, probchoice] = rdm_svd_model(data)
