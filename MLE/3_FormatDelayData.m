...
%% Cohort 3
rdm_versions = {'fitrdmdata_luce_3.mat', 'fitrdmdata_softmax_3.mat'};
save_names   = {'dd_luce_3.mat', 'dd_softmax_3.mat'};
rdm_fields   = {'rdm_3', 'rdm_3'}; % adjust if variable names differ inside each .mat

for v = 1:2
    % Load JSON
    raw = jsondecode(fileread('dd_3.json'));
    ns          = raw.ns;
    Tsubj       = raw.Tsubj;
    trial_ends  = cumsum(Tsubj);
    trial_starts = [1; trial_ends(1:end-1) + 1];
    ...
    for s = 1:ns
        ...
        mat(:,4) = raw.amount_later(idx);
        mat(:,5) = raw.delay_later(idx);
        mat(:,6) = 1 - raw.choice(idx); % Originally backawrds coding of choice
        mat(:,7) = rdm_alphas(rdm_idx);
