...
%% Joint Softmax Cohort 3
dd_subids = [dd_softmax_3.subid]';
for s = 1:length(joint_softmax_3)
    dd_idx = find(dd_subids == joint_softmax_3(s).subid);
    if isempty(dd_idx); continue; end
    t_rdm      = joint_softmax_3(s).data;
    t_dd       = dd_softmax_3(dd_idx).data;
    mu_true    = joint_softmax_3(s).b_joint(1);
    alpha_true = joint_softmax_3(s).b_joint(2);
    kappa_true = joint_softmax_3(s).b_joint(3);
    sv_cert        = t_rdm(:,3).^alpha_true;
    sv_lott        = t_rdm(:,5) .* t_rdm(:,4).^alpha_true;
    prob_rdm       = 1 ./ (1 + exp(-mu_true .* (sv_lott - sv_cert)));
    t_rdm_sim      = t_rdm; t_rdm_sim(:,6) = double(rand(size(t_rdm,1),1) < prob_rdm);
    ...
    t_dd_sim       = t_dd; t_dd_sim(:,6) = double(rand(size(t_dd,1),1) < prob_dd);
    result_sim                        = fitmodel_joint_softmax(t_rdm_sim, t_dd_sim);
    joint_softmax_3(s).b_joint_rec    = result_sim.b;
end
B_true = vertcat(joint_softmax_3.b_joint);
B_rec  = vertcat(joint_softmax_3.b_joint_rec);
figure;
for p = 1:3
    subplot(1,3,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
...
sgtitle('Joint Softmax - Cohort 3');
...
%% Joint Luce Cohort 3
dd_subids = [dd_luce_3.subid]';
for s = 1:length(joint_luce_3)
    dd_idx = find(dd_subids == joint_luce_3(s).subid);
    if isempty(dd_idx); continue; end
    t_rdm      = joint_luce_3(s).data;
    t_dd       = dd_luce_3(dd_idx).data;
    mu_true    = joint_luce_3(s).b_joint(1);
    alpha_true = joint_luce_3(s).b_joint(2);
    kappa_true = joint_luce_3(s).b_joint(3);
    sv_cert        = t_rdm(:,3).^alpha_true;
    sv_lott        = t_rdm(:,5) .* t_rdm(:,4).^alpha_true;
    prob_rdm       = sv_lott.^mu_true ./ (sv_lott.^mu_true + sv_cert.^mu_true);
    t_rdm_sim      = t_rdm; t_rdm_sim(:,6) = double(rand(size(t_rdm,1),1) < prob_rdm);
    ...
    t_dd_sim       = t_dd; t_dd_sim(:,6) = double(rand(size(t_dd,1),1) < prob_dd);
    result_sim                     = fitmodel_joint_luce(t_rdm_sim, t_dd_sim);
    joint_luce_3(s).b_joint_rec    = result_sim.b;
end
B_true = vertcat(joint_luce_3.b_joint);
B_rec  = vertcat(joint_luce_3.b_joint_rec);
figure;
for p = 1:3
    subplot(1,3,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
