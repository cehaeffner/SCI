function result = fitmodel_joint_luce(rdm_data, dd_data)
...
rdm_probchoice    = utilgamble.^data.mu ./ (utilgamble.^data.mu + utilcertain.^data.mu);
rdm_utildiff = nan(size(rdm_probchoice)); % not needed in luce
% rdm_logodds  = nan(size(probchoice)); % not needed in luce
...
dd_probchoice = utillate.^data.mu ./ (utillate.^data.mu + utilsoon.^data.mu);
dd_utildiff = nan(size(dd_probchoice)); % not needed in luce
% dd_logodds  = nan(size(probchoice)); % not needed in luce
