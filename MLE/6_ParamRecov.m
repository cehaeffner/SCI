...
%% RDM Softmax Cohort 3
for s = 1:length(rdm_softmax_3)
    t          = rdm_softmax_3(s).data;
    mu_true    = rdm_softmax_3(s).b_rdm(1);
    alpha_true = rdm_softmax_3(s).b_rdm(2);
    sv_cert    = t(:,3).^alpha_true;
    sv_lott    = t(:,5) .* t(:,4).^alpha_true;
    utildiff   = sv_lott - sv_cert;
    probchoice = 1 ./ (1 + exp(-mu_true .* utildiff));
    t_sim      = t; t_sim(:,6) = double(rand(size(t,1),1) < probchoice);
    result_sim                 = fitmodel_rdm_softmax(t_sim);
    rdm_softmax_3(s).b_rdm_rec = result_sim.b;
end
B_true = vertcat(rdm_softmax_3.b_rdm);
B_rec  = vertcat(rdm_softmax_3.b_rdm_rec);
figure;
for p = 1:2
    subplot(1,2,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
...
%% RDM Luce Cohort 3
for s = 1:length(rdm_luce_3)
    t          = rdm_luce_3(s).data;
    mu_true    = rdm_luce_3(s).b_rdm(1);
    alpha_true = rdm_luce_3(s).b_rdm(2);
    sv_cert    = t(:,3).^alpha_true;
    sv_lott    = t(:,5) .* t(:,4).^alpha_true;
    probchoice = sv_lott.^mu_true ./ (sv_lott.^mu_true + sv_cert.^mu_true);
    t_sim      = t; t_sim(:,6) = double(rand(size(t,1),1) < probchoice);
    result_sim               = fitmodel_rdm_luce(t_sim);
    rdm_luce_3(s).b_rdm_rec  = result_sim.b;
end
B_true = vertcat(rdm_luce_3.b_rdm);
B_rec  = vertcat(rdm_luce_3.b_rdm_rec);
figure;
for p = 1:2
    subplot(1,2,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
...
%% DD Softmax Cohort 3
for s = 1:length(dd_softmax_3)
    t          = dd_softmax_3(s).data;
    mu_true    = dd_softmax_3(s).b_dd(1);
    kappa_true = dd_softmax_3(s).b_dd(2);
    alpha      = t(1,7);
    sv_sooner  = t(:,2).^alpha ./ (1 + kappa_true .* t(:,3));
    sv_later   = t(:,4).^alpha ./ (1 + kappa_true .* t(:,5));
    utildiff   = sv_later - sv_sooner;
    ...
    t_sim      = t; t_sim(:,6) = double(rand(size(t,1),1) < probchoice);
    result_sim                 = fitmodel_ddnlh_softmax(t_sim);
    dd_softmax_3(s).b_dd_rec   = result_sim.b;
end
B_true = vertcat(dd_softmax_3.b_dd);
B_rec  = vertcat(dd_softmax_3.b_dd_rec);
figure;
for p = 1:2
    subplot(1,2,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
...
%% DD Luce Cohort 3
for s = 1:length(dd_luce_3)
    t          = dd_luce_3(s).data;
    mu_true    = dd_luce_3(s).b_dd(1);
    kappa_true = dd_luce_3(s).b_dd(2);
    alpha      = t(1,7);
    sv_sooner  = t(:,2).^alpha ./ (1 + kappa_true .* t(:,3));
    sv_later   = t(:,4).^alpha ./ (1 + kappa_true .* t(:,5));
    probchoice = sv_later.^mu_true ./ (sv_later.^mu_true + sv_sooner.^mu_true);
    t_sim      = t; t_sim(:,6) = double(rand(size(t,1),1) < probchoice);
    result_sim               = fitmodel_ddnlh_luce(t_sim);
    dd_luce_3(s).b_dd_rec    = result_sim.b;
end
B_true = vertcat(dd_luce_3.b_dd);
B_rec  = vertcat(dd_luce_3.b_dd_rec);
figure;
for p = 1:2
    subplot(1,2,p); scatter(B_true(:,p), B_rec(:,p), 40, 'filled'); hold on; refline(1,0);
    [r,pv] = corr(B_true(:,p), B_rec(:,p));
