...
for s = 1:length(rdm_3)
    t = rdm_3(s).data;
    result_rdm = fitmodel_rdm_softmax(t);
    rdm_3(s).result_rdm = result_rdm;
    rdm_3(s).b_rdm      = result_rdm.b;
    rdm_3(s).pr2_rdm    = result_rdm.pseudoR2;
end

% Add model-free data
for s = 1:length(rdm_3)
    rdm_3(s).pGamble = mean(rdm_3(s).data(:,6));
end

% Save
save('fitrdmdata_softmax_3.mat', 'rdm_3');

% Sanity check: alpha v pGam
B = vertcat(rdm_3.b_rdm);
pGamble = [rdm_3.pGamble]';
...
sgtitle('Cohort 3 Softmax RDM Model: Parameter distributions');

% Sanity check: pR2 distribution
figure('Color', 'w');
pr2_numeric = [rdm_3.pr2_rdm];
histogram(pr2_numeric, 80, 'FaceColor', [0.7 0.7 0.7], 'EdgeColor', 'w');
xlim([0,1])
xline(0, 'black', 'Chance', 'LineWidth', 2, 'LabelHorizontalAlignment', 'left');
xline(mean(pr2_numeric, 'omitnan'), '-b', sprintf('Mean (%.2f)', mean(pr2_numeric, 'omitnan')), 'LineWidth', 2);
xline(median(pr2_numeric, 'omitnan'), '-r', sprintf('Median (%.2f)', median(pr2_numeric, 'omitnan')), 'LineWidth', 2);
title('Cohort3Softmax Model Fit (pR^2)'); xlabel('Predictive Pseudo-R^2'); ylabel('Count'); axis square;
n_below_0  = sum(pr2_numeric < 0);
n_below_01 = sum(pr2_numeric < 0.1);
...
for s = 1:length(rdm_3)
    ...
    sv_diff = sv_lott - sv_safe;
    choice  = t(:,6);
    rdm_3(s).sv_diff = sv_diff;
    rdm_3(s).choice  = choice;
end

all_sv  = vertcat(rdm_3.sv_diff);
all_ch  = vertcat(rdm_3.choice);
...
plot(binCents, pChoice, 'ko-', 'LineWidth', 2, 'MarkerFaceColor', 'k');
yline(0.5, 'k--'); ylim([0,1])
xlabel('SV difference (gamble - cert)'); ylabel('P(gamble)'); title('Cohort 3 Softmax Choice by SV difference (10 bins)');
axis square;
...
% Save
save('fitrdmdata_luce_3.mat', 'rdm_3');
...
sgtitle('Cohort 3 Luce RDM Model: Parameter distributions');
...
plot(binCents, pChoice, 'ko-', 'LineWidth', 2, 'MarkerFaceColor', 'k');
yline(0.5, 'k--'); ylim([0,1])
xlabel('SV difference (gamble - cert)'); ylabel('P(gamble)'); title('Cohort 3 Luce Choice by SV difference (10 bins)');
axis square;
